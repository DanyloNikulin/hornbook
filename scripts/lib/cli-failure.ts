...
import { emitJobEvent } from './job-protocol.ts';
...
export function retainFailedCleanup(error: unknown): boolean {
  ...
  process.exitCode = 1;
  const report = (failure: Error) => emitJobEvent({ type: 'cleanup', error: failure.message });
  report(error);
  console.error(
    'Process cleanup is still pending. Retry cleanup in Jobs, or press Ctrl+C in this terminal.',
  );
  const keepAlive = setInterval(() => undefined, 60_000);
  const retry = () => {
    void error.retryCleanup().catch(report);
  };
